import { nativeToString, variableTypeDetection } from './verifyType';
import { Callback, IAnyObject } from '@web-watch/types';
import { globalVar, ToStringTypes } from "@web-watch/shared";
...
export function validateOption(target: any, targetName: string, expectType: string): any {
  ...
  console.error(`web-watch: ${targetName}期望传入${expectType}类型，目前是${typeofAny(target)}类型`);
}
